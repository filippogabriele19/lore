...
def _check_expr_taint(node, tainted_vars: Set[str], src_bytes: bytes) -> Tuple[bool, str]:
    if node.type == "identifier":
        ...
        if any(pat in func_text for pat in ("query", "get", "body", "cookie", "request", "header", "params")):
...
class TSASTTaintTracer(BaseTaintTracer):
    ...
    def trace(self) -> Dict:
        try:
            ...
            print(f"⚠️ Failed to parse TS file for taint analysis: {e}")
    ...
    def _walk(self, node):
        ...
        # 1. Parameter declaration in TS/JS
        if t in ("required_parameter", "optional_parameter", "parameter"):
            # A parameter declaration may just be an identifier
            for child in node.children:
        ...
        # 2. Assignment / Variable declarator
        elif t == "variable_declarator":
            # const x = val
            left = node.child_by_field_name("name") or node.children[0]
        ...
        # 3. Call expression
            ...
            # Sinks
            sink_names = {"eval", "exec", "execsync", "spawn", "spawnsync", "run", "query", "execute"}
...
def check_ts_taint_interprocedural(source_code: str, external_sources: Set[str] | None = None) -> Dict:
    tracer = TSASTTaintTracer(source_code, external_sources)
    return tracer.trace()
